kmeansReplaceTRUE <- function (x, centers, iter.max = 10L, nstart = 1L, algorithm = c("Hartigan-Wong", 
    "Lloyd", "Forgy", "MacQueen"), trace = FALSE) 
{
    ...
    do_one <- function(nmeth) {
        switch(nmeth, {
            ...
            Z <- .Fortran(stats:::C_kmns, x, m, p, centers = centers,   # https://stackoverflow.com/questions/10253759/unable-to-find-c-kmns-object-when-passed-to-fortran
            ...
            Z <- .C(stats:::C_kmeans_MacQueen, x, m, p, centers = as.double(centers), 
                k, c1 = integer(m), iter = iter.max, nc = integer(k), 
                wss = double(k))
        })
    ...
    if (length(centers) == 1L) {
        k <- centers
        if (nstart == 1L) 
            centers <- x[sample.int(m, k, replace=TRUE), , drop = FALSE]
        if (nstart >= 2L || any(duplicated(centers))) {
            cn <- unique(x)
            mm <- nrow(cn)
            if (mm < k) 
                stop("more cluster centers than distinct data points.")
            centers <- cn[sample.int(mm, k, replace=TRUE), , drop = FALSE]
        }
    }
    else {
