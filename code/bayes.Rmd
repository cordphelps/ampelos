...
ave.df$population <- rnorm(nrow(ave.df), mean=15, sd=5)
...
# R code 10.40
ave.df$log_pop <- log(ave.df$population)
ave.df$contact_high <- ifelse( ave.df$contact=="oakMargin" , 1 , 0 )
...
plotWeekly <- function(df, t) {


  ggplot(df, aes(x=week, y=aveSpiders, colour=cluster)) + 
      ...
      labs(title=paste(t, " transect: ave spiders\ntrapped per day", sep=""),
        subtitle=paste("", sep=""), 
          y="average spiders per observation", 
          x="week", 
          caption = paste("", sep="") ) +
