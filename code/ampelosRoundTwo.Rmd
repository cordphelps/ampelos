...
```{r stats, echo=FALSE, include=FALSE, results='hide', message=F, warning=T}
...
observations.daytime.period1 <- combo.period1.tibl %>% group_by(julian) %>% count()
observations.daytime.period2 <- combo.period2.tibl %>% group_by(julian) %>% count()
observations.daytime.period3 <- combo.period3.tibl %>% group_by(julian) %>% count()

# to get observations in the period, multiply observations per day times days in the period
# ( observations per day should always be '60' )
...
stats.tbl <- tribble(~period, ~cohens.d, ~sig.level, ~power, ~impliedSampleSize,  ~actualSampleSize,
                     "period1", round(cohens.d.period1$d,2), 0.05, 0.80, round(power.period1$n,0),
                     total.obs.daytime.period1,
                     "period2", round(cohens.d.period2$d,2), 0.05, 0.80, round(power.period2$n,0),
                     total.obs.daytime.period2,
                     "period3", round(cohens.d.period3$d,2), 0.05, 0.80, round(power.period3$n,0),
