...
rl <- returnList
  ...
  if (TRUE) {
    ...
    gg.likelihood.pm <- generateLikelihoodV2(df=filtered.df, inboundList=rl, daytime='pm')
    print(gg.likelihood.pm)
    
  
    if (FALSE) {
  
    filtered.df <- rl[[4]] %>% dplyr::filter(time == 'am')
    gg.likelihood.am <- generateLikelihoodV2(df=filtered.df, inboundList=rl, daytime='am')
    print(gg.likelihood.am) # likelihood by cluster with seasonal timeframes
  
  
    gg.likelihood.24 <- generateLikelihoodV2(df=rl[[4]], inboundList=rl, daytime='24h')
    print(gg.likelihood.24)
    ...
    ggsave("right.4.1.pdf", plot = gg.likelihood.pm, device = NULL, path = "./IALE/poster_files",
       scale = 1, width = 6, height = NA, dpi = 300, limitsize = TRUE,
       units = c("in", "cm", "mm"))
